...
class _KPlistBuilder(object):
    ...
    # IMPLEMENTATION IDEA:
    # The XMLTreeBuilder calls us at four points: when there's a new start tag,
    # when there's a new end tag, when there's data from a tag, and when
    # there's no more data. We build objects incrementally out of these
    # notifications. Each tag type can implement handlers for the start and end
    # tags. Exactly one of these handlers must return an object that represents
    # the parsed plist entry. Collection entries must return the object from
    # the start tag handler, while leaf entries must return the object from the
    # end tag handler. Once a handler has produced an object for the plist
    # entry, that object gets added to the result using add_object. Collections
    # are maintained in a collection stack. When a start tag handler returns an
    # object, that object is pushed onto the top of the collection stack to
    # indicate that it is the current collection. When an end tag handler does
    # not return a value, that indicates that the current collection is done
    # and the collection stack is popped. When the ID attribute is encountered,
    # the subsequent call to add_object associates the object with that ID.
    # When a corresponding IDREF attribute is encountered, the start and end
    # tag handlers are skipped. Instead, once the next end tag is received, the
    # previous object is looked up by ID and passed to add_object.
    ...
    def start(self, tag, attr):
        ...
        for attrname in set(attr.keys()).difference(('ID', 'IDREF')):
            if attrname not in self.attributes[tag]:
                raise ValueError(f'illegal attribute "{attrname}" for tag "{tag}"')
        ...
        if self.current_idref is not None:
            ...
            if tag != original_tag:
                raise ValueError(f'tag "{tag}" has IDREF to element with different tag "{original_tag}"')
    ...
    def get_id_attr(self, attr, name):
        ...
        if id_attr is not None:
            try:
                ...
                raise ValueError(f'invalid {name} attribute')
    ...
    def add_object(self, tag, value):
        ...
        if tag == 'key':
            ...
            if self.current_key in self.collection_stack[-1]:
                raise ValueError(f'duplicate key "{self.current_key}" in dict')
